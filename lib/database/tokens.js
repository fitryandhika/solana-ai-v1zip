...
/**
 * Tokens that don't have a logo image yet — either discovered before the
 * image feature existed, or DexScreener simply didn't have artwork at
 * discovery time. Bounded by `limit` so a backfill pass never fires an
 * unbounded number of lookups.
 */
async function getTokensMissingImage(limit = 100) {
  const supabase = getServiceClient();
  ...
  if (error) {
    throw new Error(`getTokensMissingImage failed: ${error.message}`);
  }
...
/**
 * Updates just the image_url for one token — used by the backfill pass, so
 * it doesn't touch any other field on the row.
 */
async function updateTokenImage(address, imageUrl) {
  const supabase = getServiceClient();

  const { error } = await supabase.from("tokens").update({ image_url: imageUrl }).eq("address", address);

  if (error) {
    throw new Error(`updateTokenImage failed for ${address}: ${error.message}`);
  }
}

/**
 * Marks an existing token row as watchlisted (used when the address is
 * already tracked — no need to re-fetch/re-insert anything).
 */
async function setTokenWatchlisted(address, note) {
  const supabase = getServiceClient();

  const { error } = await supabase
    .from("tokens")
    .update({ watchlisted: true, watchlist_note: note || null, watchlisted_at: new Date().toISOString() })
    .eq("address", address);

  if (error) {
    throw new Error(`setTokenWatchlisted failed for ${address}: ${error.message}`);
  }
}

/**
 * Removes a token from the watchlist — does NOT delete the token or its
 * tracking history, just un-flags it. The scanner keeps tracking it
 * regardless (watchlisting isn't the only reason a token gets tracked).
 */
async function removeTokenFromWatchlist(address) {
  const supabase = getServiceClient();

  const { error } = await supabase
    .from("tokens")
    .update({ watchlisted: false, watchlist_note: null })
    .eq("address", address);

  if (error) {
    throw new Error(`removeTokenFromWatchlist failed for ${address}: ${error.message}`);
  }
}

/**
 * All currently watchlisted tokens (with pair_address) — used by the
 * scanner to pick up newly-watchlisted tokens it isn't tracking yet
 * without requiring a restart.
 */
async function getWatchlistedAddresses() {
  const supabase = getServiceClient();

  const { data, error } = await supabase.from("tokens").select("address, pair_address").eq("watchlisted", true);

  if (error) {
    throw new Error(`getWatchlistedAddresses failed: ${error.message}`);
  }
