...
async function insertWalletTrade(trade) {
  ...
  // Touch the wallet row so it exists (source defaults to 'system') and
  // its last_trade_at stays current — actual scoring happens separately
  // on a periodic pass, not on every single trade insert.
  await supabase
    .from("wallets")
