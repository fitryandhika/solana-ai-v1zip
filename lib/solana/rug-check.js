...
async function checkMintAndFreezeAuthority(mintAddress) {
  try {
    ...
    const info = await throttledCall(() => connection.getParsedAccountInfo(new PublicKey(mintAddress)));
...
async function getTop10HolderPercent(mintAddress) {
  try {
    ...
    const largest = await throttledCall(() => connection.getTokenLargestAccounts(mintPubkey));
    const supply = await throttledCall(() => connection.getTokenSupply(mintPubkey));
