...
// This module's RPC calls share the SAME Helius API key (and its 10
// requests/second budget) as lib/solana/websocket.js's transaction lookups.
// Firing rug-check's 3 calls all at once — as this used to do via
// Promise.all with no throttle at all — could momentarily consume the
// entire shared budget and cause the blockchain discovery stream's own
// (separately throttled) lookups to 429 at the same time. Spacing out
// every RPC call this module makes keeps the two consumers from colliding.
const MIN_RPC_INTERVAL_MS = Number(process.env.RUG_CHECK_MIN_RPC_INTERVAL_MS || 350);
let lastRpcCallAt = 0;

async function throttle() {
  const now = Date.now();
  const wait = lastRpcCallAt + MIN_RPC_INTERVAL_MS - now;
  if (wait > 0) {
...
/**
 * Checks whether mint authority and freeze authority have been revoked
 * (set to null) for a given SPL token mint.
 * Returns { mintAuthorityRevoked, freezeAuthorityRevoked } — each true,
 * false, or null if the check could not be completed.
 */
...
/**
 * Returns the percentage of total supply held by the 10 largest holder
 * accounts, or null if it could not be determined. Uses
 * getTokenLargestAccounts, which returns at most the top 20 by protocol
 * design — no need to enumerate every holder.
 */
...
/**
 * Runs all discovery-time checks for one token. Best-effort — a failure in
 * one check doesn't block the others. Meant to be called ONCE per token, at
 * discovery, not on every snapshot (authority/holder concentration doesn't
 * need re-checking every few seconds).
 */
async function runDiscoveryChecks(mintAddress) {
  const authorities = await checkMintAndFreezeAuthority(mintAddress);
  const top10HolderPct = await getTop10HolderPercent(mintAddress);
